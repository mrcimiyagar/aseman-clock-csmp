...
class JsonHubProtocol implements csmp.signalr.HubProtocol {
    ...
    @Override
    public csmp.signalr.TransferFormat getTransferFormat() {
        return csmp.signalr.TransferFormat.TEXT;
    }

    @Override
    public csmp.signalr.HubMessage[] parseMessages(String payload, InvocationBinder binder) {
        if (payload.length() == 0) {
            return new csmp.signalr.HubMessage[]{};
        }
        ...
        List<csmp.signalr.HubMessage> hubMessages = new ArrayList<>();
        try {
            for (String str : messages) {
                csmp.signalr.HubMessageType messageType = null;
                ...
                do {
                    ...
                    switch (name) {
                        case "type":
                            messageType = csmp.signalr.HubMessageType.values()[reader.nextInt() - 1];
                            break;
                        case "invocationId":
                ...
                switch (messageType) {
                    ...
                    case STREAM_ITEM:
                        ...
                        hubMessages.add(new csmp.signalr.StreamItem(invocationId, result));
                        break;
                    case STREAM_INVOCATION:
                    case CANCEL_INVOCATION:
                        throw new UnsupportedOperationException(String.format("The message type %s is not supported yet.", messageType));
                    case PING:
                        hubMessages.add(csmp.signalr.PingMessage.getInstance());
                        break;
                    case CLOSE:
                        if (error != null) {
                            hubMessages.add(new csmp.signalr.CloseMessage(error));
                        } else {
                            hubMessages.add(new csmp.signalr.CloseMessage());
                        }
                        break;
                    default:
                        break;
        ...
        return hubMessages.toArray(new csmp.signalr.HubMessage[hubMessages.size()]);
    }

    @Override
    public String writeMessage(csmp.signalr.HubMessage hubMessage) {
