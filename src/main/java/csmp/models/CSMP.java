...
import csmp.callbacks.network.ServerCallback;
import csmp.callbacks.network.ServerCallback2;
import csmp.callbacks.terminal.OnBotViewInitialized;
import csmp.callbacks.terminal.OnFileMessageCreated;
import csmp.callbacks.terminal.OnTextMessageCreated;
import com.fasterxml.jackson.core.type.TypeReference;
import csmp.helpers.*;
import csmp.models.*;
import csmp.retrofit.BotHandler;
import csmp.retrofit.ComplexHandler;
import csmp.retrofit.MessageHandler;
import csmp.retrofit.NotifHandler;
import retrofit2.Call;
import csmp.rxbus.Bus;
import csmp.rxbus.notifications.*;
import csmp.signalr.HubConnection;
import csmp.signalr.HubConnectionBuilder;
import csmp.signalr.HubConnectionState;
...
public class CSMP {
    ...
    public CSMP(long sessionId, String token) {
        ...
        connection.on("NotifyUserClickedBotView", CSMP.this::onUserClickedBotView, Notifications.UserClickedBotViewNotification.class);
        connection.on("NotifyUserRequestedBotPreview", CSMP.this::onUserRequestedBotPreview, Notifications.UserRequestedBotPreviewNotification.class);
        connection.on("NotifyBotLoaded", CSMP.this::onBotLoaded, Notifications.BotLoadedNotification.class);
        connection.on("NotifyBotViewResized", CSMP.this::onBotViewResized, Notifications.BotViewResizedNotification.class);
    }
    ...
    public void initBotView(boolean windowMode, long complexId, long roomId, long userId, Controls.Control view, OnBotViewInitialized callback) {
        Packet packet = new Packet();
        Entities.Complex complex = new Entities.Complex();
        complex.setComplexId(complexId);
        packet.setComplex(complex);
        ...
        packet.setUser(user);
        packet.setBotWindowMode(windowMode);
        try {
    ...
    public void initBotViewToAll(boolean windowMode, Controls.Control view) {
        for (Entities.Workership workership : DatabaseHelper.getWorkerships()) {
            Entities.BaseRoom room = workership.getRoom();
            ...
            for (Entities.Membership membership : members) {
                ...
                initBotView(windowMode, complex.getComplexId(), room.getRoomId(), user.getBaseUserId(), view, () -> {});
            }
        }
    }

    public void updateBotView(boolean windowMode, long complexId, long roomId, long userId, Updates.Update update) {
        Packet packet = new Packet();
        Entities.Complex complex = new Entities.Complex();
        complex.setComplexId(complexId);
        packet.setComplex(complex);
    ...
    public void updateBotViewToAll(boolean windowMode, Updates.Update update) {
        for (Entities.Workership workership : DatabaseHelper.getWorkerships()) {
            Entities.BaseRoom room = workership.getRoom();
            Entities.Complex complex = room.getComplex();
            List<Entities.Membership> members = complex.getMembers();
            for (Entities.Membership membership : members) {
                Entities.User user = membership.getUser();
                updateBotView(windowMode, complex.getComplexId(), room.getRoomId(), user.getBaseUserId(), update);
            }
        }
    }

    public void updateBotView(boolean windowMode, long complexId, long roomId, long userId, List<Updates.Update> updates) {
        Packet packet = new Packet();
        Entities.Complex complex = new Entities.Complex();
        complex.setComplexId(complexId);
        packet.setComplex(complex);
        ...
        packet.setBotWindowMode(windowMode);
        try {
            String json = JsonHelper.getMapper().writerFor(new TypeReference<List<Updates.Update>>(){}).writeValueAsString(updates);
            packet.setRawJson(json);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public void updateBotViewToAll(boolean windowMode, List<Updates.Update> updates) {
        for (Entities.Workership workership : DatabaseHelper.getWorkerships()) {
            Entities.BaseRoom room = workership.getRoom();
            Entities.Complex complex = room.getComplex();
            List<Entities.Membership> members = complex.getMembers();
            for (Entities.Membership membership : members) {
                Entities.User user = membership.getUser();
                updateBotView(windowMode, complex.getComplexId(), room.getRoomId(), user.getBaseUserId(), updates);
            }
        }
    }

    public void animateBotView(boolean windowMode, long complexId, long roomId, long userId, Anims.Anim animation) {
        Packet packet = new Packet();
        Entities.Complex complex = new Entities.Complex();
        complex.setComplexId(complexId);
        packet.setComplex(complex);
    ...
    public void animateBotViewToAll(boolean windowMode, Anims.Anim anim) {
        for (Entities.Workership workership : DatabaseHelper.getWorkerships()) {
            Entities.BaseRoom room = workership.getRoom();
            Entities.Complex complex = room.getComplex();
            List<Entities.Membership> members = complex.getMembers();
            for (Entities.Membership membership : members) {
                Entities.User user = membership.getUser();
                animateBotView(windowMode, complex.getComplexId(), room.getRoomId(), user.getBaseUserId(), anim);
            }
        }
    }

    public void animateBotView(boolean windowMode, long complexId, long roomId, long userId, List<Anims.Anim> anims) {
        Packet packet = new Packet();
        Entities.Complex complex = new Entities.Complex();
        complex.setComplexId(complexId);
        packet.setComplex(complex);
        ...
        packet.setBotWindowMode(windowMode);
        try {
            String json = JsonHelper.getMapper().writerFor(new TypeReference<List<Anims.Anim>>(){}).writeValueAsString(anims);
            packet.setRawJson(json);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public void animateBotViewToAll(boolean windowMode, List<Anims.Anim> anims) {
        for (Entities.Workership workership : DatabaseHelper.getWorkerships()) {
            Entities.BaseRoom room = workership.getRoom();
            Entities.Complex complex = room.getComplex();
            List<Entities.Membership> members = complex.getMembers();
            for (Entities.Membership membership : members) {
                Entities.User user = membership.getUser();
                animateBotView(windowMode, complex.getComplexId(), room.getRoomId(), user.getBaseUserId(), anims);
            }
        }
    }

    public void runCommandOnBotView(boolean windowMode, long complexId, long roomId, long userId, Codes.Code code) {
        Packet packet = new Packet();
        Entities.Complex complex = new Entities.Complex();
        complex.setComplexId(complexId);
        packet.setComplex(complex);
    ...
    public void runCommandOnBotViewToAll(boolean windowMode, Codes.Code command) {
        for (Entities.Workership workership : DatabaseHelper.getWorkerships()) {
            Entities.BaseRoom room = workership.getRoom();
            Entities.Complex complex = room.getComplex();
            List<Entities.Membership> members = complex.getMembers();
            for (Entities.Membership membership : members) {
                Entities.User user = membership.getUser();
                runCommandOnBotView(windowMode, complex.getComplexId(), room.getRoomId(), user.getBaseUserId(), command);
            }
        }
    }

    public void runCommandsOnBotView(boolean windowMode, long complexId, long roomId, long userId, List<Codes.Code> codes) {
        Packet packet = new Packet();
        Entities.Complex complex = new Entities.Complex();
        complex.setComplexId(complexId);
        packet.setComplex(complex);
    ...
    public void runCommandsOnBotViewToAll(boolean windowMode, List<Codes.Code> commands) {
        for (Entities.Workership workership : DatabaseHelper.getWorkerships()) {
            Entities.BaseRoom room = workership.getRoom();
            Entities.Complex complex = room.getComplex();
            List<Entities.Membership> members = complex.getMembers();
            for (Entities.Membership membership : members) {
                Entities.User user = membership.getUser();
                runCommandsOnBotView(windowMode, complex.getComplexId(), room.getRoomId(), user.getBaseUserId(), commands);
            }
        }
    }
    ...
    public void start() {
        NetworkHelper.requestServer(NetworkHelper.getRetrofit().create(ComplexHandler.class).botGetWorkerships(), new ServerCallback2() {
            ...
            public void onRequestSuccess(Packet packet) {
                ...
                if (packet.getWorkerships() == null)
                    DatabaseHelper.setWorkerships(new ArrayList<>());
                else
    ...
    private void onUserClickedBotView(Notifications.UserClickedBotViewNotification notif) {
        LogHelper.log("Aseman", "User Clicked Bot View notification");

        BusHelper.bus().post(new UserClickedBotView(notif.getComplex(), notif.getRoom(), notif.getUser(), notif.getControlId()));

        notifyServerNotifReceived(notif.getNotificationId());
    }

    private void onBotLoaded(Notifications.BotLoadedNotification notif) {
        LogHelper.log("Aseman", "Bot View Loaded notification");

        BusHelper.bus().post(new BotLoaded(notif.getComplexId(), notif.getRoomId(), notif.getUserId(), notif.isBotWindowMode()));

        notifyServerNotifReceived(notif.getNotificationId());
    }

    private void onBotViewResized(Notifications.BotViewResizedNotification notif) {
        LogHelper.log("Aseman", "Bot View Resized notification");

        BusHelper.bus().post(new BotViewResized(notif.getUserId(), notif.getComplexId(), notif.getRoomId()
                , notif.getNewWidth(), notif.getNewHeight()));

        notifyServerNotifReceived(notif.getNotificationId());
    }

    private void onUserRequestedBotPreview(Notifications.UserRequestedBotPreviewNotification notif) {
        LogHelper.log("Aseman", "User Requested Bot View notification");

        BusHelper.bus().post(new UserRequestedBotView(0, 0, true,
                notif.getWorkerWidth(), notif.getWorkerHeight(), notif.getUser(), false));
    ...
    private void onUserRequestedBotView(Notifications.UserRequestedBotViewNotification notif) {
        ...
        BusHelper.bus().post(new UserRequestedBotView(notif.getComplexId(), notif.getRoomId(), false,
                notif.getWorkerWidth(), notif.getWorkerHeight(), notif.getUser(), notif.isBotWindowMode()));
